...
TODAY = min(datetime.datetime.now(), datetime.datetime(2022, 12, 25)).day
...
def main() -> None:
    ...
    #     const="big.txt",
    #     default="input.txt",
    #     help="use the big inputs from https://the-tk.com/project/aoc2021-bigboys.html",
    # )
    ...
    for i, day_arg in enumerate(days):
        n = int(day_arg.partition("_")[0])
        path = ROOT / f"input/day{n}" / args.input_name
        if not path.exists() and path.with_suffix(".txt.xz").exists():
            path = path.with_suffix(".txt.xz")
            file = lzma.open(path, "rt")
        else:
            file = open(path, "r")  # pylint: disable=consider-using-with
        try:
